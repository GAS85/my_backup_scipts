...
ArchiveDir=$WhereToMount_$(date +"%Y-%m-%d")
...
if [ "$CompressToArchive" == true ]; then
	...
	if [ -f "$WhereToMount/$RemoteArchiveName" ]; then
		...
		archivemount $WhereToMount/$RemoteArchiveName $ArchiveDir

		echo Rsync of NC into Archive
		rsync $RsyncOptions $excludeFromBackup $NextCloudPath $ArchiveDir
